...
abstract class IExpandable<T>(
    ...
    override fun draw(x: Float, y: Float, mouseX: Float, mouseY: Float): Float {
        ...
        val bh = hovered(buttonX, y + 8f, buttonW, 20f)

        `anim$button`.value = if (bh) Mocha.Surface1.argb else Mocha.Base.argb
        ...
        if (expanded || containerHeight > 0f) {
            ...
            for ((i, option) in options.withIndex()) {
                ...
                val oh = hovered(x + 6f, optionY, width - 12f, optionHeight)

                `anim$optionHover`[i].value = if (oh) Mocha.Surface1.argb else Mocha.Base.argb
    ...
    override fun mouseClicked(mouseX: Float, mouseY: Float, button: Int): Boolean {
        ...
        if (hovered(buttonX, lastY + 8f, buttonW, 20f)) {
            toggleExpanded()
            return true
        }
        ...
        if (expanded) {
            for (i in options.indices) {
                ...
                if (hovered(lastX + 6f, optionY, width - 12f, optionHeight)) {
                    click(i)
                    return true
                }
            }
